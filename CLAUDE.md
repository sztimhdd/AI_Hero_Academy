...
- **2026-03** — Claude repeatedly ran `pip install playwright` + `python -m playwright install chromium` when asked to do browser testing. Root cause: no rule said to use the Playwright MCP server instead. The MCP server (`mcp__playwright__browser_*`) is already registered and handles the browser process — no local Python playwright package is needed. **Rule**: never `pip install playwright`; call `mcp__playwright__browser_*` tools directly in the main session.
- **2026-03** — MCP tools (`mcp__brave-search__*`, `mcp__context7__*`, `mcp__playwright__*`, etc.) returned `No such tool available` when called inside sub-agents (Agent tool). Root cause: the Databricks proxy strips `tool_reference` blocks required for sub-agent tool discovery. `ENABLE_TOOL_SEARCH=true` in settings.json does not fix this — the proxy drops the blocks before the model sees them. **Rule**: all MCP tool calls must be made directly in the main session. Never delegate MCP calls to sub-agents. There is no workaround until Databricks updates their proxy.
- **2026-03** — ECC (`/skill-health`, `/harness-audit`) crashed with `Cannot find module skills-health.js` because `CLAUDE_PLUGIN_ROOT` env var was not set. Also discovered only 26/108 ECC skills were installed (original install ran minimal subset). **Rule**: `CLAUDE_PLUGIN_ROOT` must be set in `settings.json` `env` to `C:/tmp/everything-claude-code`. To update ECC skills: `cd /c/tmp/everything-claude-code && git pull && node scripts/install-apply.js --modules framework-language,database,workflow-quality,agents-core,commands-core` — never include `rules-core` (overwrites custom Databricks rule edits).
